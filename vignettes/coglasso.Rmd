...
To select the best combination of hyperparameters, we will use `select_coglasso()`, setting `method` to "xstars" (which is the default behaviour). This function implements *eXtended StARS* (*XStARS*, [Albanese, Kohlen and Behrouzi, 2024](#ref)) a coglasso-adapted version of *StARS*, the stability selection method developed by Liu, Roeder and Wasserman ([2010](#ref)). The usual application of `select_coglasso()` uses the default options of the function, whose only required input is the object resulting from `coglasso()`.
